...
const briefSchema = z.object({
  ...
  brand: z.object({
    ...
    primaryColor: z.string().regex(/^#[0-9A-Fa-f]{6}$/).default("#173F7B"),
    secondaryColor: z.string().regex(/^#[0-9A-Fa-f]{6}$/).default("#4A90D9"),
...
export function buildSiteContentFromBrief(brief: BriefData): SiteContent {
  ...
  return {
    ...
    theme: {
      ...
      starterKit: brief.brand.starterKit || starterKit?.key || "",
      accentColor: brief.brand.primaryColor || starterKit?.theme?.primaryColor || "#173F7B",
      accentAltColor: brief.brand.secondaryColor || starterKit?.theme?.secondaryColor || "#4A90D9",
    },
